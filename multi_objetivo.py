...
# Novas constantes
epsilon_0 = 0.05                 # Precisão global desejada (exemplo)
...
# ======================================================================
class FederatedLearningOptimization(ElementwiseProblem):
...
# ======================================================================
problem = FederatedLearningOptimization(N, alpha, c, S, f_min, f_max, epsilon_0, theta_prev)
...
res = minimize(
    ...
    save_history=True,
    verbose=True,
    termination=('n_gen', 50) # Mais gerações costumam ser necessárias para 3 objetivos
)
...
if res.F is not None:
    ...
    for i in range(min(5, len(res.X))):
        ...
        for n in range(N):
            ...
            print(f"  n={n}: f_{n} = {val_f:.4e} | beta_{n} = {val_beta} | psi_{n} = {val_psi} | theta_{n} = {val_theta:.4f}")
